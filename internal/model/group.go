...
type Group struct {
	...
	OwnerID   uint      `gorm:"index;not null" json:"owner_id"`   // 群主 ID
	Announce  string    `gorm:"type:text" json:"announce"`         // 群公告
...
type GroupMember struct {
	...
	MutedUntil *time.Time `json:"muted_until,omitempty"`               // 禁言截止时间，nil 表示未禁言
	CreatedAt  time.Time `json:"created_at"`
	UpdatedAt  time.Time `json:"updated_at"`
}
